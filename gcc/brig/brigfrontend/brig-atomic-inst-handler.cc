...
brig_atomic_inst_handler::atomic_builtins_map brig_atomic_inst_handler::atomic_builtins_;

brig_atomic_inst_handler::brig_atomic_inst_handler (brig_to_generic& parent) :
	brig_code_entry_handler (parent)
{
	if (atomic_builtins_.size () > 0) return;
	...
	add_custom_atomic_builtin
		("__phsa_builtin_atomic_min_s32", 2,
		 integer_type_node, ptr_type_node, integer_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_min_s64", 2,
		 long_integer_type_node, ptr_type_node, long_integer_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_min_u32", 2,
		 uint32_type_node, ptr_type_node, uint32_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_min_u64", 2,
		 uint64_type_node, ptr_type_node, uint64_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_max_s32", 2,
		 integer_type_node, ptr_type_node, integer_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_max_u32", 2,
		 uint32_type_node, ptr_type_node, uint32_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_max_s64", 2,
		 long_integer_type_node, ptr_type_node, long_integer_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_max_u64", 2,
		 uint64_type_node, ptr_type_node, uint64_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_wrapdec_u32", 2,
		 uint32_type_node, ptr_type_node, uint32_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_wrapdec_u64", 2,
		 uint64_type_node, ptr_type_node, uint64_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_wrapinc_u32", 2,
		 uint32_type_node, ptr_type_node, uint32_type_node);

	add_custom_atomic_builtin
		("__phsa_builtin_atomic_wrapinc_u64", 2,
		 uint64_type_node, ptr_type_node, uint64_type_node);
}

void
brig_atomic_inst_handler::add_custom_atomic_builtin (
...
size_t
brig_atomic_inst_handler::generate_tree (
    const BrigInstBase &inst,	BrigAtomicOperation8_t atomic_opcode)
{
  ...
  tree instr_type = get_tree_type_for_hsa_type (inst.type);
	...
	tree atomic_type =
		build_qualified_type (instr_type, TYPE_QUAL_ATOMIC);
	...
	if (atomic_opcode == BRIG_ATOMIC_ST)
	...
	else if (atomic_opcode == BRIG_ATOMIC_CAS)
		{
			...
			atomic_builtins_map::iterator i = atomic_builtins_.find (builtin_name.str ());
			...
			instr_expr = call_builtin (&built_in, NULL, 3, instr_type,
																 ptype, ptr,
																 src0_type, src0,
																 src1_type, src1);
		}
	else
		{
			...
			switch (atomic_opcode)
				...
				case BRIG_ATOMIC_ADD:
					builtin_name << "__sync_fetch_and_add";	break;
				case BRIG_ATOMIC_SUB:
					builtin_name << "__sync_fetch_and_sub";	break;
				case BRIG_ATOMIC_AND:
					builtin_name << "__sync_fetch_and_and";	break;
				case BRIG_ATOMIC_XOR:
					builtin_name << "__sync_fetch_and_xor";	break;
				case BRIG_ATOMIC_OR:
					builtin_name << "__sync_fetch_and_or"; break;
				case BRIG_ATOMIC_EXCH:
					builtin_name << "__sync_lock_test_and_set";	break;
				default:
					break;
				}
					...
					switch (atomic_opcode)
						...
						case BRIG_ATOMIC_WRAPDEC:
							...
							switch (atomic_opcode)
								{
								case BRIG_ATOMIC_MIN:
									builtin_name << "min_"; break;
								case BRIG_ATOMIC_MAX:
									builtin_name << "max_"; break;
								case BRIG_ATOMIC_WRAPINC:
									builtin_name << "wrapinc_"; break;
								case BRIG_ATOMIC_WRAPDEC:
									builtin_name << "wrapdec_"; break;
								default:
									break;
								}
			...
			instr_expr = call_builtin (&built_in, NULL, 2, instr_type,
																 ptr_type_node, ptr, arg0_type, src0);
			...
			tree tmp_assign = build2
				(MODIFY_EXPR, TREE_TYPE (tmp_var), tmp_var, instr_expr);
			parent_.append_statement (tmp_assign);
			instr_expr = tmp_var;
		}
