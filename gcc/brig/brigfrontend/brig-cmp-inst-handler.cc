...
brig_cmp_inst_handler::operator() (const BrigBase *base)
{
  ...
  const bool is_fp16_dest = (inst_base->type & 0x01F) == BRIG_TYPE_F16;
	...
	bool is_int_cmp =
		...
		INTEGRAL_TYPE_P (TREE_TYPE (cmp_type)) : INTEGRAL_TYPE_P (cmp_type);
		...
		case BRIG_COMPARE_NE:
			if (is_int_cmp)
			...
			else
				{
					...
					neg_expr =
						build2 (UNEQ_EXPR, result_type, operands [1], operands [2]);
				}
			break;
  ...
  if (expr == NULL_TREE && neg_expr == NULL_TREE)
		internal_error
			("unimplemented comparison  %x for type %x",
			 inst->compare, inst->sourceType);
	...
	if (is_fp16_dest)
					 ...
					 !is_boolean_dest)
		{
			...
			tree shift_left_result = build2
				(LSHIFT_EXPR, signed_type, signed_result,
				 build_int_cstu (signed_type, result_width - 1));

			expr = build2
				(RSHIFT_EXPR, signed_type, shift_left_result,
				 build_int_cstu (signed_type, result_width - 1));
		}
