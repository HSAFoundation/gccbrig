...
struct reg_use_info
{
  /* The generic tree types as the registers is interpreted and their
     "score" (a count times referred as the type and some added
     weigth). The score is used to select type for 'register'
     variables to reduce reinterpret_casts emission. The data is kept
     in vector (insertion order) for determinism, in a case there is a
     tie with the scores.  */
...
/* key = hsa register entry generated by gccbrig_hash_hsa_reg ().  */
typedef std::map<size_t, reg_use_info> regs_use_index;

size_t gccbrig_hsa_reg_hash (const BrigOperandRegister &reg);
std::string gccbrig_hsa_reg_name_from_hash (size_t reg_hash);

void gccbrig_print_reg_use_info (const regs_use_index &info);
