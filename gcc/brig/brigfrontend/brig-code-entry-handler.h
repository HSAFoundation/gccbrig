...
protected:
  ...
  tree get_tree_cst_for_hsa_operand (
  ...
  tree_code get_tree_code_for_hsa_opcode (
	...
	tree build_tree_operand (
      const BrigInstBase &brig_inst, const BrigBase &operand, tree operand_type=NULL_TREE,
			bool is_input=false);

	tree build_address_operand (
      const BrigInstBase &brig_inst, const BrigOperandAddress &addr_operand);

	tree build_tree_operand_from_brig	(
      const BrigInstBase *brig_inst, tree operand_type, size_t operand_index);

	tree build_tree_cst_element (
  ...
  tree build_h2f_conversion (tree source);
	...
	tree apply_to_all_elements (
      tree_element_unary_visitor &visitor, tree operand);
...
private:
	...
	tree add_custom_builtin (
      BrigOpcode16_t brig_opcode, BrigType16_t itype,const char* name,
...
public:
	...
	virtual tree visit_element (
      brig_code_entry_handler& handler, tree operand) = 0;
};
...
public:
	...
	// been a vector element or a scalar.
	virtual tree visit_element (
      brig_code_entry_handler& handler, tree operand0, tree operand1) = 0;
};
...
public:
	...
	virtual tree visit_element (
      brig_code_entry_handler& caller, tree operand);
private:
...
public:
  ...
  size_t operator() (const BrigBase *base);
	...
	tree build_variable	(
      const BrigDirectiveVariable *brigVar, tree_code var_decl_type=VAR_DECL);
};
...
protected:
	...
	size_t generate_tree (
      const BrigInstBase &inst,	BrigAtomicOperation8_t atomic_opcode);

private:

	void add_custom_atomic_builtin (const char *name, int nargs,
																	tree rettype, ...);
