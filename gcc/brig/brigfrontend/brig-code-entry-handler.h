...
class brig_code_entry_handler : public brig_entry_handler {
 ...
 protected:
  ...
  tree build_tree_operand (const BrigInstBase &brig_inst,
			   const BrigBase &operand, tree operand_type=NULL_TREE,
			   bool is_input=false);
  ...
  tree build_output_assignment (const BrigInstBase &brig_inst, tree output, tree inst_expr);

  // Applies the given Visitor to all (vector) elements of the operand.
  // Scalars are considered single element vectors.
  tree apply_to_all_elements (tree_element_unary_visitor &visitor, tree operand);
