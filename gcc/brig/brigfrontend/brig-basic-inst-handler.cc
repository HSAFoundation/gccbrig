...
public:
  ...
  virtual tree visit_element
  (brig_code_entry_handler&, tree operand0, tree operand1)
  {
    ...
    return call_builtin
      (&builtin, builtin_name_.c_str (), 2,
       TREE_TYPE (operand0),
       TREE_TYPE (operand0), operand0, TREE_TYPE (operand1), operand1);
  }
...
bool
brig_basic_inst_handler::must_be_scalarized
(const BrigInstBase *brig_inst, tree instr_type) const
{
  if (!VECTOR_TYPE_P (instr_type)) return false;
  if (!brig_inst->opcode == BRIG_OPCODE_MULHI) return false;
  ...
  BrigType16_t element_type = brig_inst->type & 0x01F;
  if (elements < 16 &&
      (element_type == BRIG_TYPE_S8 ||
       element_type == BRIG_TYPE_U8 ||
       element_type == BRIG_TYPE_S16 ||
       element_type == BRIG_TYPE_U16))
    return true;
  else
    return false;
}
...
tree
brig_basic_inst_handler::build_shuffle
(tree arith_type, tree_stl_vec& operands)
{
  ...
  for (size_t i = 0; i < TYPE_VECTOR_SUBPARTS (arith_type); ++i)
	...
	offset =
	  build_int_cst
	  (element_type, TYPE_VECTOR_SUBPARTS (arith_type));
  ...
  tree mask_vec_type =
    build_vector_type (element_type,
		       TYPE_VECTOR_SUBPARTS (arith_type));
  ...
  tree perm = build3
    (VEC_PERM_EXPR, TREE_TYPE (operands[0]), operands[0],
     operands[1], mask);
  return perm;
}
...
brig_basic_inst_handler::build_pack (tree_stl_vec& operands)
{
  ...
  tree wide_type =
    build_nonstandard_integer_type (vecsize, 1);

  tree src_vect =
    build_reinterpret_cast (wide_type, operands [0]);
  ...
  pos = add_temp_var ("pos",
		      convert
		      (wide_type,
		       build2 (BIT_AND_EXPR, TREE_TYPE (pos), operands [2],
  ...
  uint64_t mask_int = element_width == 64 ? (uint64_t)-1 :
    ((uint64_t)1 << element_width) - 1;
  ...
  tree element_in_position =
    build2 (LSHIFT_EXPR, wide_type,
	    build2 (BIT_AND_EXPR, wide_type, scalar, mask),
	    bitoffset);
...
tree
brig_basic_inst_handler::build_unpack_lo_or_hi
(BrigOpcode16_t brig_opcode, tree arith_type, tree_stl_vec& operands)
{
  ...
  size_t offset =
    (brig_opcode == BRIG_OPCODE_UNPACKLO) ?	0 : element_count / 2;
...
tree
brig_basic_inst_handler::build_instr_expr
 ...
 tree arith_type, tree_stl_vec& operands)
{
  ...
  tree instr_inner_type = VECTOR_TYPE_P (arith_type) ?
    TREE_TYPE (arith_type) : arith_type;
  ...
  if (opcode == RSHIFT_EXPR || opcode == LSHIFT_EXPR)
	...
	build_vector_from_val (arith_type, scalar_mask) : scalar_mask;

      // The shift amount is a scalar -- broadcast it to produce
      // a vector shift.
      if (VECTOR_TYPE_P (arith_type))
	operands [1] =
	  build_vector_from_val	(arith_type, operands [1]);
  ...
  if (opcode == TREE_LIST)
    {
      ...
      if (brig_opcode == BRIG_OPCODE_MAD ||
	  brig_opcode == BRIG_OPCODE_MAD24)
	{
	  ...
	  tree mult_res = build2 (MULT_EXPR, arith_type,
      ...
      else if (brig_opcode == BRIG_OPCODE_NRSQRT)
	{
	  ...
	  return build2
	    (RDIV_EXPR, arith_type, build_one_cst (arith_type),
	     expand_or_call_builtin (BRIG_OPCODE_SQRT, brig_type, arith_type, operands));
	}
      else if (brig_opcode == BRIG_OPCODE_NRCP)
	{
	  ...
	  // native ISA other than a division, if available.
	  return build2
	    (RDIV_EXPR, arith_type, build_one_cst (arith_type), operands [0]);
	}
    ...
    return expand_or_call_builtin
      (brig_opcode, brig_type, arith_type, operands);
...
brig_basic_inst_handler::operator()(const BrigBase *base)
{
  ...
  size_t output_count = first_input;
  size_t input_count = operands.size() == 0 ?
    0 : (operands.size() - output_count);

  gcc_assert (output_count == 0 || output_count == 1);

  tree_stl_vec::iterator first_input_i = operands.begin();
  if (output_count > 0 && operands.size() > 0) ++first_input_i;
  ...
  tree_code opcode = get_tree_code_for_hsa_opcode
    (brig_inst->opcode, brig_inst_type);

  bool is_fp16_operation =
    (brig_inst_type & 0x01F) == BRIG_TYPE_F16 &&
  ...
  if (p == BRIG_PACK_PS || p == BRIG_PACK_PSSAT)
    in_operands[1] =
      build_lower_element_broadcast (in_operands[1]);
  else if (p == BRIG_PACK_SP || p == BRIG_PACK_SPSAT)
    in_operands[0] =
      build_lower_element_broadcast (in_operands[0]);
  ...
  if (p >= BRIG_PACK_PPSAT && p <= BRIG_PACK_PSAT)
	...
	{
	  tree goto_stmt = build1 (GOTO_EXPR, void_type_node,
				   parent_.m_cf->exit_label);
      ...
      else
	{
	  ...
	  if (parent_.m_cf->ret_value != NULL_TREE)
	    {
	      tree result_assign = build2
		(MODIFY_EXPR, TREE_TYPE (parent_.m_cf->ret_value),
		 parent_.m_cf->ret_value, parent_.m_cf->ret_value);
	  ...
	  else
	    {
	      ...
	      parent_.append_statement (return_stmt);
	    }
	  return base->byteCount;
	}
    }
  else if (opcode == MULT_HIGHPART_EXPR && scalarize)
    {
      ...
      */
      ...
      if (input_count > 0) unpack (in_operands[0], operand0_elements);

      tree_stl_vec operand1_elements;
      if (input_count > 1) unpack (in_operands[1], operand1_elements);
	...
	{
	default:
	case BRIG_TYPE_S8: promoted_type = short_integer_type_node; break;
	case BRIG_TYPE_U8: promoted_type = short_unsigned_type_node; break;
	case BRIG_TYPE_S16: promoted_type = integer_type_node; break;
	case BRIG_TYPE_U16: promoted_type = unsigned_type_node; break;
	}
      ...
      for (size_t i = 0; i < TYPE_VECTOR_SUBPARTS (arith_type); ++i)
	{
	  ...
	  scalar_expr = build2
	    (RSHIFT_EXPR, promoted_type, scalar_expr,
  ...
  if (p == BRIG_PACK_SS || p == BRIG_PACK_S ||
      p == BRIG_PACK_SSSAT || p == BRIG_PACK_SSAT)
    {
	...
	old_value = build_reinterpret_cast
	  (TREE_TYPE (instr_expr), operands[0]);
      ...
      tree mask_inner_type =
	build_nonstandard_integer_type (esize, 1);
      ...
      for (size_t i = 0; i < element_count; ++i)
	{
      ...
      tree assign = build2
	(MODIFY_EXPR, TREE_TYPE (instr_expr), new_value, instr_expr);
      parent_.append_statement (assign);

      instr_expr = build3
	(VEC_PERM_EXPR, arith_type, old_value, new_value, mask);

      tree lower_output = create_tmp_var (TREE_TYPE (instr_expr), "s_output");
      tree assign_lower = build2
	(MODIFY_EXPR, TREE_TYPE (instr_expr), lower_output, instr_expr);
      parent_.append_statement (assign_lower);
  ...
  if (output_count == 1)
    ...
    parent_.append_statement (instr_expr);
  return base->byteCount;
}
...
brig_basic_inst_handler::build_lower_element_broadcast (tree vec_operand)
{
  ...
  return build3
    (VEC_PERM_EXPR, TREE_TYPE (vec_operand), vec_operand, vec_operand, mask);
}
